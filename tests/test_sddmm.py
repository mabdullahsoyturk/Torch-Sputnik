...
def sddmm(m, k, n, nnz):
    a = torch.arange(1, nnz + 1, dtype=torch.float32).view(m, k).cuda()
    mask, row_indices, row_offsets, column_indices, nnzs = dense_to_sparse(a)

    lhs_matrix = torch.arange(1,nnz + 1, dtype=torch.float32).view(k, n).cuda()
    rhs_matrix = torch.arange(1,nnz + 1, dtype=torch.float32).view(k, n).cuda()

    output_values = torch_sputnik.sddmm(m, k, n, nnzs, row_indices, row_offsets, column_indices, lhs_matrix, rhs_matrix, mask)
