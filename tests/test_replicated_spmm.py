...
def tensor_spmm(m, k, n, nnz):
    ...
    values2, row_indices2, row_offsets2, column_indices2 = dense_to_sparse(a)
    ...
    b = torch.arange(1, 2 * (nnz + 1) - 1).view(2, k, n).cuda().to(torch.float32)
    bias = torch.zeros((2, n)).cuda()
    c = torch.zeros((2, m, n)).cuda()
